...
public:
       ...
       // 1 -> both s1's and s2's char matches s3's ==> help(s1,s2,s3, l1-1, l2-1, l3-1) ==> pick either of the paths (the best one, i.e, we'll try to find if we get a true from either of the paths)

       // 2 -> s1's char matches s3's ==> help(s1, s2, s3, l1-1, l2, l3-1)
       // 3 -> s2's char matches s3's ==> help(s1,s2,s3, l1, l2-1, l3-1)

       bool help(string &s1, string &s2, string &s3, int l1, int l2, int l3, vector<vector<vector<int>>> &dp) {
              ...
              if (l1 >= 0 and l2 >= 0 and s1[l1] == s3[l3] and s2[l2] == s3[l3]) {
                     bool first, second;
                     first = help(s1, s2, s3, l1 - 1, l2, l3 - 1, dp);
                     second = help(s1, s2, s3, l1, l2 - 1, l3 - 1, dp);
       ...
       bool isInterleave(string s1, string s2, string s3) {
              ...
              return help(s1, s2, s3, l1 - 1, l2 - 1, l3 - 1, dp);
       }
};
